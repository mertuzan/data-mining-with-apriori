...
public class Eastbound {
    ...
    String ruleKey = "east";
    String nonKey = "west";
    String ruleFile = "nodes.csv";
    int TotalKey = 0;

    public Eastbound(String uri, String user, String password) {
    ...
    private void addNode() {
        try (Session session = driver.session()) {
            try (Transaction tx = session.beginTransaction()) {
                ...
                tx.run("MERGE (a:Wheel {name:'wheel2'})");
                tx.run("MERGE (a:Wheel {name:'wheel3'})");
                tx.run("MERGE (a:Direction {name:'east'})");
                tx.run("MERGE (a:Direction {name:'west'})");
                tx.run("MERGE (a:jagged {name:'jagged'})");
                tx.run("MERGE (a:closed {name:'closed'})");
                tx.run("MERGE (a:open_car {name:'open_car'})");
                tx.run("MERGE (a:tr_double {name:'tr_double'})");
                tx.run("LOAD CSV FROM \"file:///tr_types.csv\" AS line\n"
                        + "MERGE (n:Length {name:line[0]})\n"
                        + "RETURN n");
                tx.run("LOAD CSV FROM \"file:///shape.csv\" AS line\n"
                        + "MERGE (n:Shape {name:line[0]})\n"
                        + "RETURN n");
                tx.run("LOAD CSV FROM \"file:///load.csv\" AS line\n"
                        + "MERGE (n:Load {name:line[0]})\n"
                        + "RETURN n");

                tx.run("LOAD CSV FROM \"file:///Names.csv\" AS line\n"
                        + "MERGE (n:Car {name:line[0]})\n"
                        + "RETURN n");
                tx.success();
            }
        }
    ...
    private void addRelation(String d[], String l[]) {
        String relation;
        ...
        try (Session session = driver.session()) {
            ...
            try (Transaction tx = session.beginTransaction()) {
                for (int i = 1; i < l.length; i++) {
                    if (d[i].equals("true")) {
                        relation = "MATCH (a:" + l[0] + " {name:'" + d[0] + "'}),(b:" + l[i] + " {name:'" + l[i] + "'})\n"
                                + "MERGE(a)-[:" + l[i] + "]->(b)";
                    } else {
                        relation = "MATCH (a:" + l[0] + " {name:'" + d[0] + "'}),(b:" + l[i] + " {name:'" + d[i] + "'})\n"
                                + "MERGE(a)-[:" + l[i] + "]->(b)";
                    }
                    tx.run(relation);
                }
    ...
    private static void relation(Eastbound db2) throws IOException {

        File file = new File("eastbound.csv");
    ...
    public static void main(String... args) throws FileNotFoundException, IOException {
        //Server connection
        Eastbound db = new Eastbound("bolt://localhost:7687", "neo4j", "123"); //Change this to your own parameters
